...
export default function Home() {
  ...
  async function initializeUser() {
    await login(); // triggers the authentication flow
  }
  ...
  return (
    <>
      ...
      <main className="min-h-screen">
        ...
        >
          ...
          <AnimatedSection className="relative z-30 text-center text-white px-6 py-20 md:py-32">
            ...
            <p className="text-xl text-gray-300 max-w-2xl mx-auto mb-8">
              Connect with real users who want to help. Run incentivized
              campaigns to gather feedback, build community, and scale
              sustainably.
            </p>
            <button
        ...
        {/* CTA Section */}
        <section className="max-w-7xl mx-auto px-6 py-20">
          <AnimatedSection className="bg-card border border-border p-12 text-center">
            <h2 className="text-3xl font-bold mb-4">Ready to get started?</h2>
            <p className="text-muted-foreground mb-8 max-w-2xl mx-auto">
              Join thousands of builders and users already using Arden to grow
              their products.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={() => router.push("/dashboard")}
                className="flex items-center justify-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-opacity"
              >
                <Rocket size={20} />
                Create Campaign
              </button>
              <button
                onClick={() => router.push("/dashboard")}
                className="flex items-center justify-center gap-2 px-6 py-3 bg-muted text-foreground font-semibold hover:bg-muted/80 transition-colors"
              >
                Browse Campaigns
                <ArrowRight size={20} />
              </button>
